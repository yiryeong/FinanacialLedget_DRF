...
def expenditure(request):
    ...
    if request.method == 'GET':
        ...
        if so == 'recent':
            expenditure_list = Expenditure.objects.filter(uid=user, pay_date__range=[start_date, search_date]).order_by('-pay_date')
        else:
            expenditure_list = Expenditure.objects.filter(uid=user, pay_date__range=[start_date, search_date]).order_by('pay_date')
        ...
        # paging
        paginator = Paginator(serializer.data, 5)  # 페이지당 5개씩 보여 주기
        page_obj = paginator.get_page(page)

        context = {'expenditure_list': page_obj, 'category_list': category_list, "selected_category": int(selected_category),
                   "search_date": search_date, "date_scope": date_scope, "kw": kw, "so": so}
